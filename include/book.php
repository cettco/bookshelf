<?php
        ...
        for($i = 1;$i<=$count;$i++){
            ...
            if($i%3==0){
                ...
                $var=$var."<div><div class='sample thumb3 cover' name='$title' style='$url'>$title</div></div></div></div>";
                $m++;
            }
            else if($i%3==1){
                $title = $row['title'];
                ...
                $url ="background-image: url(http://127.0.0.1:9999/bookshelf/include/$img);";
                $name = "http://127.0.0.1:9999/bookshelf/include/data/".$userid."/".$title.".1";
                $var=$var."<div class='row'><div class='loc'><div><div class='sample thumb2 cover' name='$title' style='$url'>$title</div></div>";
            }
            else{
                $title = $row['title'];
                $img ="data/".$title.".1/0.png";
                $url ="background-image: url(http://127.0.0.1:9999/bookshelf/include/$img);";
                $name = "http://127.0.0.1:9999/bookshelf/include/data/".$userid."/".$title.".1";
                $var=$var."<div><div class='sample thumb2 cover' name='$title' style='$url'>$title</div></div>";
            }
        }   
        mysql_close($bd2);
        echo $var;
